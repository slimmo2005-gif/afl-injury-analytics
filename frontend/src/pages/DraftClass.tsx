...
export default function DraftClass() {
  ...
  const draft = data?.draftClass

  if (loading) {
    return <p className="text-slate-400">Loading draft class data…</p>
  }

  if (error || !data || !draft?.players.length) {
    return (
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-6 text-amber-200">
        ...
        <Link to="/current" className="inline-block mt-4 text-sm text-afl-gold hover:text-yellow-300">
          ← Back to live season
        </Link>
      </div>
    )
    ...
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-slate-400">
          {draft.debuted} of {draft.totalPicks} have played AFL in {draft.season} · performance score is
          stats-only (no draft-potential boost)
        </p>
      </header>
